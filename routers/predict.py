...
from model.plantvillage_predictor import predict_plantvillage

router = APIRouter(tags=["Unified Prediction"])
...
async def predict_image(
    ...
    try:
        ...
        if model_type == ModelType.plantdoc:
            result = predict_plantdoc(contents)
        elif model_type == ModelType.plantvillage:
            result = predict_plantvillage(contents)
        else:
            return JSONResponse(status_code=400, content={"error": "Invalid model_type."})
